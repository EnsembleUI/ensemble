...
void main() {
  ...
  group('Default App Tests', () {
    ...
    testWidgets("Test Widgets inheriting from themes", (tester) async {
      ...
      expect(originalText.decoration?.border is UnderlineInputBorder, true);    // default is underline
      expect((originalText.decoration?.border as UnderlineInputBorder).borderRadius, BorderRadius.circular(5));
      expect(originalText.decoration?.border?.borderSide.color, const Color(0xFF404040));
      expect(originalText.decoration?.border?.borderSide.width, 1.0);
      expect(originalText.decoration?.focusedBorder?.borderSide.color, const Color(0xFF49DEFF));
      ...
      expect((updatedStyleText.decoration?.border as UnderlineInputBorder).borderRadius, BorderRadius.circular(10));
      expect(updatedStyleText.decoration?.border?.borderSide.width, 2.0);
      expect(updatedStyleText.decoration?.border?.borderSide.color, const Color(0xFFCFA17B));
      expect(updatedStyleText.decoration?.focusedBorder?.borderSide.color, const Color(0xFF86BAA6));

      // find Input with Box style
      widget = find.ancestor(
          of: find.text('box style'),
          matching: find.byType(TextField));
      expect(widget, findsOneWidget);
      TextField boxStyleText = tester.firstWidget(widget);
      expect(boxStyleText.decoration?.border is OutlineInputBorder, true);
      expect((boxStyleText.decoration?.border as OutlineInputBorder).borderRadius, BorderRadius.circular(99));
