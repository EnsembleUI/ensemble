...
class StorageManager {
  ...
  static const systemPreviewKey = 'system.preview';  // for Preview mode or regular
  ...
  Future<void> write(String key, dynamic value) => GetStorage().write(key, value);
  ...
  void updateUser(BuildContext context, String id, {String? name, String? email, String? photo}) {
    ...
    if (name != null) {
      ...
      ScreenController().dispatchSystemStorageChanges(context, 'name', name, storagePrefix: 'user');
    } else if (systemStorage.hasData(userNameKey)) {
      systemStorage.remove(userNameKey);
      ScreenController().dispatchSystemStorageChanges(context, 'name', null, storagePrefix: 'user');
    }

    if (email != null) {
      systemStorage.write(userEmailKey, email);
      ScreenController().dispatchSystemStorageChanges(context, 'email', email, storagePrefix: 'user');
    } else if (systemStorage.hasData(userEmailKey)) {
      systemStorage.remove(userEmailKey);
      ScreenController().dispatchSystemStorageChanges(context, 'email', null, storagePrefix: 'user');
    }

    if (photo != null) {
      systemStorage.write(userPhotoKey, photo);
      ScreenController().dispatchSystemStorageChanges(context, 'photo', photo, storagePrefix: 'user');
    } else if (systemStorage.hasData(userPhotoKey)) {
      systemStorage.remove(userPhotoKey);
      ScreenController().dispatchSystemStorageChanges(context, 'photo', null, storagePrefix: 'user');
    }
  }
