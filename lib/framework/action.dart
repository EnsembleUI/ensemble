...
class StartTimerAction extends EnsembleAction {
  StartTimerAction({super.initiator, required this.onTimer, this.onTimerComplete, this.payload});
  ...
  factory StartTimerAction.fromYaml({Invokable? initiator, YamlMap? payload}) {
    ...
    return StartTimerAction(
        initiator: initiator, onTimer: onTimer, onTimerComplete: onTimerComplete, payload: timerPayload);
  }
}
...
class ExecuteCodeAction extends EnsembleAction {
  ExecuteCodeAction(
      {super.initiator, super.inputs, required this.codeBlock, this.onComplete, required this.codeBlockSpan});
  ...
  factory ExecuteCodeAction.fromYaml({Invokable? initiator, YamlMap? payload}) {
    ...
    return ExecuteCodeAction(
        ...
        onComplete: EnsembleAction.fromYaml(payload['onComplete'], initiator: initiator),
...
class ShowToastAction extends EnsembleAction {
  ...
  factory ShowToastAction.fromYaml({YamlMap? payload}) {
    if (payload == null || (payload['message'] == null && payload['widget'] == null)) {
      throw LanguageError("${ActionType.showToast.name} requires either a message or a widget to render.");
    }
...
class GetLocationAction extends EnsembleAction {
  GetLocationAction({this.onLocationReceived, this.onError, this.recurring, this.recurringDistanceFilter});
...
class TimerPayload {
  TimerPayload({this.id, this.startAfter, required this.repeat, this.repeatInterval, this.maxTimes, this.isGlobal});
...
abstract class EnsembleAction {
  ...
  static EnsembleAction? fromYaml(dynamic action, {Invokable? initiator}) {
    if (action is YamlMap) {
      ...
      if (actionType != null) {
        ...
        return ExecuteCodeAction(initiator: initiator, codeBlock: action, codeBlockSpan: ViewUtil.optDefinition(null));
      }
    }
    return null;
  }
  ...
  static EnsembleAction? fromActionType(ActionType actionType, {Invokable? initiator, YamlMap? payload}) {
    if (actionType == ActionType.navigateScreen) {
      return NavigateScreenAction.fromYaml(initiator: initiator, payload: payload);
    } else if (actionType == ActionType.navigateModalScreen) {
      return NavigateModalScreenAction.fromYaml(initiator: initiator, payload: payload);
