...
class DiagramController extends WidgetController {
  ...
  ImageNode createImageNode(String name, String imagePath, Size size, {Offset? position,Subtree? subtree}) {
  ...
  Future<void> init() async {
    if ( _nodes.isEmpty ) {
      ...
      ImageNode node = createImageNode('node0','assets/images/img_placeholder.png',const Size(170,107),position:const Offset(10,0));
      node.subtree = Subtree([
        CompositeNode('compnode1', [
          createImageNode('img1', 'assets/images/img_placeholder.png', const Size(50,50)),
          TextNode('mytext1','2.4 GHz'),
          ],gap:0, layout: 'horizontal'),
        CompositeNode('compnode2', [
          createImageNode('img1', 'assets/images/img_placeholder.png', const Size(50,50)),
          TextNode('mytext2','+5 more WiFi devices'),
        ],gap:0, layout: 'horizontal'),

          ],
          ConnectionPoint(node,'right','center',0),
          distanceFromParent: 150
      );
      m['node0'] = node;

      m['node1'] =
          createImageNode('node1','assets/images/img_placeholder.png',const Size(170,107),position:const Offset(10,127));
      m['node2'] =
          createImageNode('node2','assets/images/img_placeholder.png',const Size(15,23),position:const Offset(500, 380));
      m['node3'] =
          createImageNode('node3','assets/images/img_placeholder.png',const Size(15,23),position:const Offset(500, 423));
      m['node4'] =
          createImageNode('node4','assets/images/img_placeholder.png',const Size(15,23),position:const Offset(500, 476));


          Node node5 = createImageNode('node5','assets/images/img_placeholder.png',const Size(170,107),position:const Offset(500, 120),);
          node5.subtree = Subtree([
              ...
              CompositeNode('compnode2', [
                ...
                ConnectionPoint(node5,'right','center',0),
                distanceFromParent: 250
            );
      m['node5'] = node5;
      Node node6 = createImageNode('node5','assets/images/img_placeholder.png',const Size(170,107),position:const Offset(500, 240));
      node6.subtree = Subtree([
        ...
        CompositeNode('compnode2', [
          ...
          ConnectionPoint(node6,'right','center',0),
          distanceFromParent: 250
      );
      m['node6'] = node6;
  ...
  List<Connection> get connections {
    if ( _connections.isEmpty ) {
      _connections.add(Connection(
          ConnectionPoint(
              nodes['node1']!, 'bottom', 'center', -15
          ),
          ConnectionPoint(
              nodes['node4']!, 'left', 'center', 0
          ),
          Colors.orange,
          strokeWidth: 2,
          style:'kinked'
      ...
      _connections.add(Connection(
          ConnectionPoint(
              nodes['node1']!, 'bottom', 'center', 0
          ),
          ConnectionPoint(
              nodes['node3']!, 'left', 'center', 0
          ),
          Colors.orange,
          strokeWidth: 2,
          style:'kinked'
      ...
      _connections.add(Connection(
          ConnectionPoint(
              nodes['node0']!, 'right', 'end', -30
          ),
          ConnectionPoint(
              nodes['node5']!, 'left', 'center', 0
          ),
          Colors.grey,
          strokeWidth: 2,
          style:'kinked'
      ...
      _connections.add(Connection(
          ConnectionPoint(
              nodes['node0']!, 'right', 'end', -15
          ),
          ConnectionPoint(
              nodes['node6']!, 'left', 'center', 0
          ),
          Colors.grey,
          strokeWidth: 2,
          style:'kinked'
...
class Diagram extends StatefulWidget with Invokable, HasController<DiagramController,DiagramState> {
...
class CompositeNode extends Node {
  ...
  String layout;
  double gap;
  CompositeNode(String name,this.children,{Offset? position,
      this.layout = 'vertical',this.gap = 5,Subtree? subtree}) : super(name,position:position,subtree:subtree);

  @override
  draw(Canvas canvas) {
    if ( position == null ) {
      throw Exception('Position is null when trying to to draw compositenode with name=$name');
    }
    ...
    for ( Node child in children ) {
      ...
      child.draw(canvas);
      if ( layout == 'vertical' ) {
        currPosition = Offset(currPosition.dx,currPosition.dy + child.size!.height + gap);
      } else {
        currPosition = Offset(currPosition.dx + child.size!.width + gap,currPosition.dy);
      }
    }
  }
}
class TextNode extends Node {
  ...
  TextNode(String name, this.text,{position,this.color=Colors.black,this.fontSize=12}):super(name,position:position);
  @override
...
class Rectangle extends Node {
  ...
  PaintingStyle style;
  Rect? rect;
  Rectangle(String name,Size size,Offset position,this.color,{this.style=PaintingStyle.fill}):
        super(name,position:position,size:size);
  Rectangle.from(String name,Offset position,Size size,Color color, [style=PaintingStyle.fill]):
...
class ImageNode extends Node {
  String imgPath;
  ui.Image? image;
  ImageNode(String name, this.imgPath,size, {position,subtree}) : super(name,size:size,position:position,subtree:subtree);
  ...
  draw(Canvas canvas) {
    if ( position == null ) {
      throw Exception('position of the ImageNode is null. Make sure node has position');
    }
    if ( image == null ) {
      return;
...
class Connection {
  ...
  Color color;
  double strokeWidth;
  Connection(this.from,this.to,this.color,{this.strokeWidth=2,this.style='straight'});
}
class DiagramPainter extends CustomPainter {
  ...
  Offset getConnectionPointCoordinates(ConnectionPoint point) {
    if ( point.node.size == null ) {
      throw Exception('size of the node is null for the connection point. Make sure node is drawn first and has size');
    }
    if ( point.node.position == null ) {
      throw Exception('position of the node is null for the connection point. Make sure node is drawn first and has position');
    }
    double x = 0;
    double y = 0;
  ...
  void paintChildren(Canvas canvas, Node node) {
    ...
    if ( node.subtree!.startPoint.side == 'right' ) {
      ...
      double startY = point.dy-((n-1)*node.subtree!.gapBetweenChildren/2);
      canvas.drawLine(
          Offset(offsetX,startY),
          Offset(offsetX,point.dy+((n-1)*node.subtree!.gapBetweenChildren/2)), paint);

      for ( int i=0;i<node.subtree!.children!.length;i++ ) {
        Offset endPoint = Offset(offsetX+node.subtree!.branchLength, startY+i*node.subtree!.gapBetweenChildren);
        canvas.drawLine(
