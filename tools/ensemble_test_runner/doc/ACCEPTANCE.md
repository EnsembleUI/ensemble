...
- **Packaging builds (ios):** `IosFtlPackager` uses `--no-tree-shake-icons`, pins `TOOLCHAINS=com.apple.dt.toolchain.XcodeDefault` (env only — no `-toolchain` flag), ObjC `INTEGRATION_TEST_IOS_RUNNER` + empty Swift unit for Xcode 26 linking, debug `--config-only` **once before** release (not after), `ENABLE_TESTABILITY=YES`, `-destination generic/platform=iOS`, absolute `-derivedDataPath`. Embeds `libXCTestBundleInject.dylib` into `Runner.app/Frameworks` when Xcode omits it, and sanitizes `.xctestrun` (strip `/usr/lib/libRPAC.dylib` DYLD inserts, disable parallelization) so FTL does not finish with **0 test case results**. Submits **one** XCTest zip (Release-iphoneos + `.xctestrun` inside). Renames `.xctestrun` SDK token to match the remote device iOS version (e.g. `26.2` → `26.3`). Auto-detects local Xcode and passes `xcodeVersion` on submit. CI pins Xcode **26.2**. Stub packages refused before submit; cloud unverified.
