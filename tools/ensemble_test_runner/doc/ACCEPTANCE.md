...
- **Packaging builds (android):** `AndroidFtlPackager` follows Flutter FTL flow (`flutter build apk --target` → `assembleAndroidTest` → `assembleDebug -Ptarget`). Requires `MainActivityTest` + `androidx.test` 1.2+ ranges. AGP may emit a ~6KiB androidTest APK (host only) when runner classes already ship in the app via `integration_test`; validation checks dex contents, not test-APK size alone.
- **Packaging builds (ios):** `IosFtlPackager` uses `--no-tree-shake-icons`, pins `TOOLCHAINS=com.apple.dt.toolchain.XcodeDefault` for Xcode 26 Metal toolchain Swift-compat linking, and `RunnerTests.swift` so the ObjC test target links Swift pods; stub packages refused before submit; cloud unverified.
