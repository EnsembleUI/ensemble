...
- **Artifact export E2E:** Android writes under `/data/local/tmp/ensemble_test_remote` and FTL `directoriesToPull` requests that same path. Host also reassembles chunked `ENSEMBLE_TEST_REMOTE_ENVELOPE_V1` from logcat (single-line envelopes exceed the ~4 KiB logcat limit), materializes screenshot chunks, prefers host `build/ensemble_test_runner` after materialize, and writes HTML + `ensemble_test_history.db` from `envelope.results`. Missing envelope still fails closed (`incomplete`). Cloud row still needs a verified green collect.
