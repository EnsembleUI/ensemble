...
- **Packaging builds (ios):** `IosFtlPackager` follows Flutter’s FTL recipe: debug `--config-only` once before release (not after), `flutter build ios --release --no-codesign --no-tree-shake-icons`, then `xcodebuild build-for-testing` with `-destination generic/platform=iOS`, `TREE_SHAKE_ICONS=NO`, absolute `-derivedDataPath`, and `TOOLCHAINS=com.apple.dt.toolchain.XcodeDefault` env only when needed (no `-toolchain` flag). No `CODE_SIGNING_*` / `ENABLE_TESTABILITY` overrides and no post-build dylib inject, `.xctestrun` sanitize, or SDK-token rename. Zips generated `Release-iphoneos` + `Runner_*.xctestrun` as-is. Fail-closed if `remote.devices[].version` ≠ SDK token from the generated filename (example/CI: **26.2** with Xcode **26.2**). ObjC `INTEGRATION_TEST_IOS_RUNNER` + empty Swift unit for Xcode 26 linking. On FTL FAILURE with 0 cases / empty GCS, logs Testing API matrix evidence. Stub packages refused before submit; cloud unverified.
