...
- **Artifact export E2E:** Local `ArtifactExportProof` covers pass/fail envelopes; Android writes under `/sdcard/googletest/test_outputfiles/ensemble_test_remote` (FTL `directoriesToPull`). Host `loadEnvelope` walks the downloaded tree. Cloud collect still needs a verified green row.
