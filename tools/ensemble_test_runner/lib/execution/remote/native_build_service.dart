...
/// `xcodebuild build-for-testing` → zip `Release-iphoneos` + `.xctestrun`.
/// Unsigned CI needs `CODE_SIGNING_*=NO`, XCTest inject dylib embed, libRPAC
/// sanitize, and catalog filename align when device OS ≠ Xcode SDK token.
class IosFtlPackager {
  ...
  Future<NativeBuildArtifacts> package({
    ...
    // Unsigned CI builds often omit XCTest inject; FTL needs it to launch.
    ...
    if (injectPath == null) {
      return _stub(
        ...
        detail:
            ...
            'the XCTest host (Infrastructure error / 0 cases).',
      );
    }
    onProgress?.call('Ensured XCTest inject dylib at $injectPath');

    // FTL pairs the *filename* SDK token with the physical device OS:
    // Xcode 26.2/26.3 emit Runner_iphoneos26.2 while the catalog OS is 26.3.
    // Also strip host-only DYLD inserts (libRPAC) that break device launch.
    ...
    final sanitize = sanitizeXctestrunForFtl(zipXctestrunFile);
    if (sanitize != null) {
