...
This Flutter example demonstrates `FlutterAppBadger.isAppBadgeSupported()`, `updateBadgeCount()`, and `removeBadge()` from the parent plugin.

## Overview

This is an example package in the Melos workspace. It depends on the parent `ensemble_app_badger` package by local path and is intended for manual verification of that package's public API.
...
```bash
melos exec --scope="ensemble_app_badger_example" -- flutter run
```
...
| --- | ---- | ----------- |
| `ensemble_app_badger` | Dependency | Parent package demonstrated by this example. |
...
- `ensemble_app_badger`: parent package used by this example.
