...
part 'ensemble_dropdown_data.dart';
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  final FocusNode _childNode = FocusNode(debugLabel: 'DropdownMenu');
...
class EnsembleDropdown<T> extends StatefulWidget {
  /// Creates a EnsembleDropdown.
  ...
  ///
  /// If the [onChanged] callback is null or the list of [EnsembleDropdown.items]
  /// is null then the dropdown button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input. A disabled button
  /// will display the [EnsembleDropdown.disabledHint] widget if it is non-null.
  /// If [EnsembleDropdown.disabledHint] is also null but [EnsembleDropdown.hint] is
  /// non-null, [EnsembleDropdown.hint] will instead be displayed.
  /// {@endtemplate}
  ...
  @override
  State<EnsembleDropdown<T>> createState() => EnsembleDropdownState<T>();
}

// ignore: public_member_api_docs
class EnsembleDropdownState<T> extends State<EnsembleDropdown<T>>
  ...
  @override
  void didUpdateWidget(EnsembleDropdown<T> oldWidget) {
  ...
  // Note: EnsembleDropdownState should be public as we need typed access to it through key.
...
/// A [FormField] that contains a [EnsembleDropdown].
///
/// This is a convenience widget that wraps a [EnsembleDropdown] widget in a
/// [FormField].
///
...
/// See also:
///
///  * [EnsembleDropdown], which is the underlying text field without the [Form]
///    integration.
class DropdownButtonFormField2<T> extends FormField<T> {
  /// Creates a [EnsembleDropdown] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
              ...
              child: Builder(
                builder: (BuildContext context) {
                  return DropdownButtonHideUnderline(
                    child: EnsembleDropdown<T>._formField(
  ...
  /// The key of EnsembleDropdown child widget
  ///
  /// This allows accessing EnsembleDropdownState.
