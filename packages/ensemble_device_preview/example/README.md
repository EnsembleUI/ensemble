...
This Flutter example demonstrates wrapping `BasicApp` in `DevicePreview` and adding a custom preview plugin.

## Overview

This is an example package in the Melos workspace. It depends on the parent `ensemble_device_preview` package by local path and is intended for manual verification of that package's public API.
...
```bash
melos exec --scope="example_ensemble_device_preview" -- flutter run
```
...
| --- | ---- | ----------- |
| `ensemble_device_preview` | Dependency | Parent package demonstrated by this example. |
...
- `ensemble_device_preview`: parent package used by this example.
