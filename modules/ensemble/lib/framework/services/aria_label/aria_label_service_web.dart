...
String _buildScript(Map<String, String> explicitLabels) {
  ...
  return "(function(){\n"
      ...
      "function run(){var nodes=document.querySelectorAll('flt-semantics[flt-tappable]');nodes.forEach(function(el){var span=el.querySelector('span');var text=(span&&span.textContent)||el.textContent||'';var label=null;map.forEach(function(v,k){if(text.toLowerCase().indexOf(k.toLowerCase())>-1){label=v;}});if(!label){label=deriveName(text);}if(label){el.setAttribute('aria-label',label);if(!el.hasAttribute('role')){el.setAttribute('role','button');}var c=el.closest('flt-semantics-container');if(c){c.setAttribute('aria-label',label);}}});}\n"
